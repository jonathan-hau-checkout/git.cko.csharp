...
public partial class CaptureCardCharge : System.Web.UI.Page
{
    ...
    protected void but_voidCharge_Click(object sender, EventArgs e)
    {
        // Create payload
        var voidChargeRequestModel = new ChargeVoid() { Description = "void charge", TrackId = "TRK12345" };
        ...
        try
        {
            ...
            // Submit your request and receive an apiResponse
            HttpResponse<Checkout.ApiServices.Charges.ResponseModels.Void> apiResponse = ckoAPIClient.ChargeService.VoidCharge(label_chargeId.Text, voidChargeRequestModel);

            if (!apiResponse.HasError)
            {
                // Access the response object retrieved from the api
                var voidinfo = apiResponse.Model;

                tb_voidChargeRsp.Text = String.Format("Status: {0}! \nCharge ID: {1} \nResponse Code: {2} \nResponse Message: {3} \nCaptured Value: {4}",
                    voidinfo.Status, voidinfo.Id, voidinfo.ResponseCode, voidinfo.ResponseMessage, voidinfo.Value);
            }
            else
            {
                // Api has returned an error object. You can access the details in the error property of the apiResponse.
                // apiResponse.error
                tb_voidChargeRsp.Text = string.Format("Message:/n{0} Error Code:/n{1} ", apiResponse.Error.Message, apiResponse.Error.ErrorCode);
            }
        }
        catch (Exception exc)
        {
